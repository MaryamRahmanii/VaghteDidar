...
const Booking = () => {
  ...
  useEffect(() => {
    const allTimes = ['۰۹:۰۰', '۱۰:۰۰', '۱۱:۰۰', '۱۵:۰۰', '۱۸:۰۰', '۱۹:۰۰'];
    const generatedSlots = allTimes.map((time, index) => {
      const isFull = (selectedDate.day * 3 + index) % 5 === 0;
      return { time, isAvailable: !isFull };
    });
              ...
              return (
                <button
                  ...
                  className={`py-2.5 rounded-xl text-sm font-bold transition-all border-2 ${
                    slot.isAvailable 
                      ? (selectedTime === slot.time ? activeClasses : availableClasses) 
                      : fullClasses
                  }`}
                >
                  {slot.time}
        ...
        <div className="bg-white dark:bg-[#1a2235] border border-blue-100 dark:border-gray-700/60 rounded-3xl p-6 sm:p-8 shadow-sm">
          ...
          <form className="space-y-5" onSubmit={handleBookingSubmit}>
            ...
            <div className="flex justify-center pt-6">
              <button 
                type="submit" 
                disabled={!selectedTime || showSuccessAlert}
              ...
              >
                {showSuccessAlert ? 'در حال تایید...' : 'تایید و رزرو وقت'}
              </button>
            </div>
