...
from app.domain.schemas.bookings import (
    BookingCreateSchema, BookingCancelSchema,
    BookingResponseSchema, BookingDetailResponseSchema
)

from app.services.booking_service import create_booking, cancel_booking
from app.infrastructure.repositories.booking_repository import get_user_bookings_detailed
...
@router.get("/bookings/me", response_model=list[BookingDetailResponseSchema])
async def my_bookings(db=Depends(get_db), user=Depends(get_current_user)):
    # Returns enriched rows with appointment_date, appointment_time,
    # meeting_title, and organizer_name joined in
    return await get_user_bookings_detailed(db, user["user_id"])
...
async def cancel_my_booking(
    ...
    return await cancel_booking(db, booking_id, "registrant", user, body.reason)